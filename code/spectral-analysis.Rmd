...
```{r}
t <- 1:length(fem_birth)
periodic_terms <- do.call(cbind, lapply(top_freq, function(freq) {
  cbind(cos(2 * pi * freq * t), sin(2 * pi * freq * t))
}))
df <- data.frame(fem_birth, t, periodic_terms)
fit_final <- lm(fem_birth ~ ., df)
plot(t, fem_birth, type="l")
lines(t, fit_final$fitted.values, lty=2, col="red")
```
Then, forecast the next 24 obervations.
```{r}
...
plot(t, fem_birth, type="l", xlim=c(0, tail(t_new, 1)))
lines(t, fit_final$fitted.values, lty=2, col="red")
